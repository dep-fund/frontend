...
export default function ProjectDocuments({ projectId, isOwner }: Props) {
  ...
  const next = () => setCurrent((c) => Math.min(documents.length - 1, c + 1));
  ...
  return (
    <div className="adv-wrapper">
      <div className="adv-header">
        <h3 className="adv-title">Documentos del Proyecto</h3>
      ...
      {showForm && (
        <div className="adv-form">
          ...
          >
            <FileText size={22} color="#EC8F41" />

            {file ? (
              <span className="adv-filename">{file.name}</span>
            ) : (
              <span>
                Arrastrá o <strong>hacé click</strong> para seleccionar
              </span>
            )}

            <span className="adv-hint">
              PDF, DOCX, XLSX, PPTX, TXT · máx. 12 MB
            </span>
          ...
          {error && <p className="adv-error">{error}</p>}
          ...
          <div className="adv-form-actions">
            <button
              ...
              disabled={uploading}
            >
              {uploading ? "Subiendo..." : "Confirmar Documento"}
            </button>
          </div>
        </div>
      )}
        ...
        <div className="adv-carousel">
          ...
          <div className="adv-cards-track">
            {documents.map((doc, i) => (
              <div
                key={doc.number}
                className={`adv-card ${i === current ? "adv-card--active" : ""}`}
              >
                <div className="adv-media">
                  {doc.url ? (
                    ...
                    >
                      <FileText size={32} color="#EC8F41" />
                      <span>Ver documento</span>
                    </a>
                  ) : (
                    <div className="adv-no-media">Sin documento</div>
                  )}
                </div>

                <div className="adv-card-body">
                  <span className="adv-number">Documento #{doc.number}</span>
                  <p className="adv-desc">{getFileName(doc.url)}</p>

                  {isOwner && (
                    <button
                      className="adv-delete-btn"
                      onClick={() => handleDelete(doc.number)}
                    >
                      <Trash2 size={13} />
                      Eliminar
                    </button>
      ...
      {documents.length > 1 && (
        <div className="adv-dots">
          {documents.map((_, i) => (
            <button
              key={i}
              className={`adv-dot ${i === current ? "adv-dot--active" : ""}`}
